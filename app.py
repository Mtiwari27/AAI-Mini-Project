...
with tab2:
    ...
    if uploaded_video is not None:
        ...
        if st.button("Process Video"):
            ...
            # Create VideoWriter with H.264 codec, which is more widely supported
            # For Windows, using XVID codec which is better supported
            ...
            frame_count = 0
            processed_frames = []  # Store frames for manual encoding if needed
            ...
            # Convert the video to a format Streamlit can display if needed
            # For Windows environments, convert AVI to MP4
            if os.name == 'nt':
                try:
                    # Use FFmpeg for conversion if available
                    import subprocess
                    subprocess.call(['ffmpeg', '-i', temp_output, '-c:v', 'libx264', '-preset', 'fast', 
                                    '-pix_fmt', 'yuv420p', output_path])
                except:
                    # If FFmpeg not available, use OpenCV to rewrite frames
                ...
                # For Linux/Mac, just rename or copy
                os.rename(temp_output, output_path)
            
            # Display processed video
            st.success("Video processing complete!")
            
            # Make sure video exists and has content
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                ...
                st.error("Failed to generate video. Displaying frames instead.")
                # As a fallback, display a sample of processed frames
                if processed_frames:
                    st.subheader("Sample processed frames:")
                    # Sample up to 5 frames to display
                    sample_indices = np.linspace(0, len(processed_frames)-1, min(5, len(processed_frames)), dtype=int)
                    for idx in sample_indices:
                        frame = cv2.cvtColor(processed_frames[idx], cv2.COLOR_BGR2RGB)
...
with tab4:
    ...
    try:
        ...
        chart_placeholder.error(f"Could not render chart: {e}")
